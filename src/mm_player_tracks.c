...
int _mmplayer_select_track(MMHandleType hplayer, MMPlayerTrackType type, int index)
{
	...
	if (type == MM_PLAYER_TRACK_TYPE_TEXT && player->subtitle_language_list)
	{
		...
		{
			debug_error("req track index is wrong");
...
int _mmplayer_get_track_language_code(MMHandleType hplayer, MMPlayerTrackType type, int index, char **code)
{
	...
	*code = (char*)malloc((language_code_size + 1) * sizeof(char));
	if (*code == NULL)
	{
		ret = MM_ERROR_PLAYER_INTERNAL;
		goto EXIT;
	}
	memset(*code, 0, (language_code_size + 1) * sizeof(char));
	debug_log ("total track num : %d , req idx : %d\n", player->selector[type].total_track_num, index);
	...
	else
	{
		...
		else
		{
			...
			sinkpad = g_ptr_array_index (player->selector[type].channels, index);
			...
			if(!tag)
			{
				...
				debug_log("there is no lang info - %s\n", *code);
			}
			else
			{
				debug_log("language information[%d] code: %s, len: %d \n", type, tag, strlen(tag));
				strncpy(*code, tag, language_code_size);
				g_free (tag);
			}
