...
#define MMPLAYER_FREEIF_GLIST(x_list) \
do \
{ \
	if (x_list) \
	{ \
...
#define MMPLAYER_ASM_CONTEXT_LOCK(x_player) \
do \
{ \
	GMutex *asm_lock = ((mm_player_t*)x_player)->sm.asm_context_lock; \
	if (asm_lock) { \
		g_mutex_lock(asm_lock); \
	} \
} while (0);
#define MMPLAYER_ASM_CONTEXT_UNLOCK(x_player) \
do \
{ \
	GMutex *asm_lock = ((mm_player_t*)x_player)->sm.asm_context_lock; \
	if (asm_lock) { \
		g_mutex_unlock(asm_lock); \
	} \
} while (0);
#if 0
