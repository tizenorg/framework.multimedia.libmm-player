...
#endif
static void	__get_exclude_element_list(mm_player_ini_t* ini, gchar* str);
...
mm_player_ini_load(mm_player_ini_t* ini)
{
	...
	if ( dict ) /* if dict is available */
	{
		...
		ini->provide_clock_for_video = iniparser_getboolean(dict, "general:provide clock for video", DEFAULT_PROVIDE_CLOCK_FOR_VIDEO);
		...
		__get_exclude_element_list(ini,
		...
		/* subtitle */
		ini->mirroring_width = iniparser_getint(dict, "subtitle:mirroring width", DEFAULT_MIRRORING_WIDTH);
		ini->mirroring_height = iniparser_getint(dict, "subtitle:mirroring height", DEFAULT_MIRRORING_HEIGHT);
		MMPLAYER_INI_GET_COLOR( dict, ini->font_color, "subtitle:font color", DEFAULT_FONT_COLOR );
		MMPLAYER_INI_GET_COLOR( dict, ini->font_background_color, "subtitle:font background color", DEFAULT_FONT_BACKGROUND_COLOR );
#endif
	}
	else /* if dict is not available just fill the structure with default value */
	{
		...
		ini->provide_clock_for_video = DEFAULT_PROVIDE_CLOCK_FOR_VIDEO;
		...
		{
			__get_exclude_element_list(ini, DEFAULT_EXCLUDE_KEYWORD);
		}
	...
	debug_log("provide_clock for video : %d\n", ini->provide_clock_for_video);
...
static
void	__get_exclude_element_list(mm_player_ini_t* ini, gchar* str)
{
	gchar** list = NULL;
	gchar** walk = NULL;
	gint i = 0;
