...
#ifdef _LITEW_OPT_
#define __MMPLAYER_LOAD_PLUGINS(x_player, x_caps_string) \
do { \
		if (__mmplayer_post_load_plugins(x_player, x_caps_string)) { \
			__mmplayer_release_factories(x_player); \
			__mmplayer_init_factories(x_player); \
		} \
}while (0)
#endif
...
static void		__mmplayer_gst_active_pad_changed(GstElement* selector, GParamSpec* unused, gpointer data);
...
int width, int height, gpointer data) // @
{
	...
	else if (player->video_stream_cb)
    	{
...
__mmplayer_gst_callback(GstBus *bus, GstMessage *msg, gpointer data) // @
{
	...
	switch ( GST_MESSAGE_TYPE( msg ) )
	{
		...
		case GST_MESSAGE_EOS:
		{
			...
			/* subtitle is sometimes longer than video data so, handle it manually */
			if (player->pipeline->textbin && player->pipeline->textbin[MMPLAYER_T_SINK].gst)
			{
				if (player->pipeline->mainbin && player->pipeline->mainbin[MMPLAYER_M_SUBSRC].gst)
				{
					GstPad *sink_pad = gst_element_get_static_pad (player->pipeline->textbin[MMPLAYER_T_SINK].gst, "sink");
					GstPad *queue_pad = gst_element_get_static_pad (player->pipeline->textbin[MMPLAYER_T_TEXT_QUEUE].gst, "sink");

					/* flush and send EOS to stop */
					if (queue_pad)
					{
						gst_pad_send_event(queue_pad, gst_event_new_flush_start());
						gst_pad_send_event(queue_pad, gst_event_new_flush_stop());
		...
		case GST_MESSAGE_BUFFERING:
		{
			...
			/* update ASM state to ASM_STATE_WAITING */
			if ((player->streamer->is_buffering == FALSE) && (MMPLAYER_CURRENT_STATE(player) != MM_PLAYER_STATE_PAUSED))
			{
		...
		case GST_MESSAGE_CLOCK_LOST:
			{
				...
				if ( ((player->ini.provide_clock_for_music) && (!player->videodec_linked))||
					((player->ini.provide_clock_for_video)&&(player->videodec_linked)))
				{
...
static void
__mmplayer_gst_active_pad_changed (GstElement* selector, GParamSpec* unused, gpointer data)
{
	GstPad* sinkpad = NULL;

	g_object_get (selector, "active-pad", &sinkpad, NULL);

	debug_log ("active-pad is changed! new active pad (%s:%s)\n", GST_DEBUG_PAD_NAME(sinkpad));
...
__mmplayer_gst_decode_pad_added (GstElement *elem, GstPad *pad, gpointer data)
{
	...
	if (strstr(name, "video"))
	{
		...
		debug_log ("%s: without input-selector\n", name);
	...
	else
	{
		...
		if (strstr(name, "audio"))
		{
			...
			if(player->is_audio_excluded) {//exclude audio decoding
		...
		if (selector == NULL)
		{
			...
			MMPLAYER_SIGNAL_CONNECT (player, selector, "notify::active-pad",
									G_CALLBACK (__mmplayer_gst_active_pad_changed), player);
...
#define ESTIMATED_BUFFER_UNIT (1*1024*1024)
		...
		__mm_player_streaming_set_queue2(player->streamer,
										player->pipeline->mainbin[MMPLAYER_M_MUXED_S_BUFFER].gst,
										TRUE,								// use_buffering
										player->streamer->buffer_handle[BUFFER_TYPE_MUXED].buffering_bytes,
										init_buffering_time,
										1.0,								// low percent
	...
	if (!audio_selector)
	{
		debug_log("[Decodebin2] there is no audio stream\n");
		...
		__mmplayer_dump_pipeline_state( player );
		MMPLAYER_GENERATE_DOT_IF_ENABLED ( player, "pipeline-status-no-audio" );
		return;
	}
...
__mmplayer_gst_decode_callback(GstElement *elem, GstPad *pad, gpointer data) // @
{
	...
	else if (strstr(name, "video"))
	{
		...
		if ( player->use_textoverlay && player->play_subtitle )
		{
			...
			{
				debug_critical("should not reach here.");
				goto ERROR;
			}
		}
...
_mmplayer_update_video_param(mm_player_t* player) // @
{
	...
	switch ( surface_type )
	{
			...
			if (!strcmp(player->ini.videosink_element_x,"xvimagesink"))
			{
				...
				if (display_method == 5)	// 5 for ROI mode
				{
					...
					g_object_set(player->pipeline->videobin[MMPLAYER_V_SINK].gst,
						...
						"dst-roi-orientation", org_angle/90,
						"dst-roi-x", roi_x,
						"dst-roi-y", roi_y,
						"dst-roi-w", roi_w,
						"dst-roi-h", roi_h,
		...
		case MM_DISPLAY_SURFACE_EVAS:
		{
			...
			/* if evasimagesink */
			if (!strcmp(player->ini.videosink_element_evas,"evasimagesink") && player->set_mode.video_zc)
			{
				int width = 0;
				int height = 0;
...
__mmplayer_audio_stream_decoded_render_cb(GstElement* object, GstBuffer *buffer, GstPad *pad, gpointer data)
{
	...
	return_if_fail(buffer && GST_BUFFER_DATA(buffer));
	return_if_fail(GST_BUFFER_SIZE(buffer) > 0);

	audio_stream.data = GST_BUFFER_DATA(buffer);
	audio_stream.data_size = GST_BUFFER_SIZE(buffer);
...
#if 0 // We'll need this in future.
/* NOTE : currently, audiosink switching is only considered. */
/* player : internal player handle
  * search_from : bin to switch element
  * removal_name : facotry name to remove
  * new_element_name : a named factory to create
  */
static int
__mmplayer_gst_switching_element(mm_player_t *player, GstElement *search_from, const gchar *removal_name, const gchar *new_element_name)
{
	int ret = MM_ERROR_NONE;
	GstElement *del_element = NULL;
	GstFormat format = GST_FORMAT_TIME;

	MMPLAYER_FENTER();
	return_val_if_fail(player && search_from && removal_name, MM_ERROR_PLAYER_NOT_INITIALIZED);
	del_element = gst_bin_get_by_name (GST_BIN (search_from), removal_name);

	if (del_element) {
		gboolean is_sink = FALSE;
		is_sink = GST_OBJECT_FLAG_IS_SET (del_element, GST_ELEMENT_IS_SINK);

		/* sink case :  ...  ! element1 ! sink element */
		if (is_sink) {
			GstPad *del_element_sink_pad = gst_element_get_static_pad(del_element, "sink");
			gboolean is_audio = TRUE;
			//GstElement *new_element = NULL;
			gchar *name = "audiosink";

			if (del_element_sink_pad) {
				GstPad* blocking_pad = gst_pad_get_peer(del_element_sink_pad);

				if (blocking_pad) {
					gint64 cur_time = 0;
					GstState new_state = GST_STATE_PAUSED;
					GstState current_state = GST_STATE(player->pipeline->mainbin[MMPLAYER_M_PIPE].gst);

					/* get current position */
					gst_element_query_position(player->pipeline->mainbin[MMPLAYER_M_PIPE].gst, &format, &cur_time);

					 if (current_state == GST_STATE_PLAYING) {
						/* 1. block src pad of element1 */
						gst_pad_set_blocked(blocking_pad, TRUE);

						new_state = GST_STATE_PLAYING;
					}

					/* 2. unlink element1 and sink element */
					if ( !gst_pad_unlink(blocking_pad, del_element_sink_pad) ) {
						debug_error("failed to unlink for switching");
						gst_object_unref(del_element_sink_pad);
						goto FAILED;
					}
					gst_object_unref(del_element_sink_pad);

					/* 3. set to NULL and remove */
					gst_element_set_state(del_element, GST_STATE_NULL);
					gst_bin_remove(GST_BIN(search_from), GST_ELEMENT(del_element));
					__mmplayer_del_sink(player, player->pipeline->audiobin[MMPLAYER_A_SINK].gst);

					/* 4. make new sink element */
					if ( !is_audio) {
						name = "videosink";
					}
					GstElement *new_element = gst_element_factory_make(new_element_name, name);

					if (new_element) {
						/* reset */
						if (is_audio) {
							player->pipeline->audiobin[MMPLAYER_A_SINK].gst = new_element;
							__mmplayer_add_sink(player, player->pipeline->audiobin[MMPLAYER_A_SINK].gst);
							g_object_set (G_OBJECT (new_element), "provide-clock", FALSE,  NULL);
						}

						/* 5. link element1 and new sink */
						GstPad *new_pad = gst_element_get_static_pad(new_element,"sink");
						if (new_pad) {
							gst_bin_add (GST_BIN(search_from), GST_ELEMENT(new_element));
							gst_element_set_state(new_element, new_state);
							gst_pad_link(blocking_pad, new_pad);
							gst_object_unref(new_pad);

							/* 6. unblock element1 */
							gst_pad_set_blocked(blocking_pad, FALSE);
							...
							debug_error("failed to get pad of new element");
							goto FAILED;
						}
					}
					else {
						gst_object_unref(blocking_pad);
						debug_error("failed to create new one");
						goto FAILED;
					}
				}
				else {
					debug_error("failed to get pad for blocking");
					goto FAILED;
				}
			}
			else {
				debug_error("failed to get pad of removal element");
				goto FAILED;
			}
		}
	}
...
__mmplayer_gst_create_audio_pipeline(mm_player_t* player)
{
	...
	else if(player->audio_stream_render_cb_ex)//audio_frame_decoded_cb
	{
		MMPLAYER_CREATE_ELEMENT(audiobin, MMPLAYER_A_SINK, "fakesink", "fakesink", TRUE);

		g_object_set (G_OBJECT (audiobin[MMPLAYER_A_SINK].gst), "sync", TRUE, NULL);
		g_object_set (G_OBJECT (audiobin[MMPLAYER_A_SINK].gst), "async", TRUE, NULL);
	...
	else // normal playback
	{
		...
		/* audio effect element. if audio effect is enabled */
		{
			...
			{
				/* Sound Booster only */
				MMPLAYER_CREATE_ELEMENT(audiobin, MMPLAYER_A_FILTER, player->ini.name_of_audio_effect, "audiofilter", TRUE);

				debug_log("audio effect config. bypass = %d, effect type  = %d", player->bypass_audio_effect, player->audio_effect_info.effect_type);
			}
		}
		/* create audio sink */
		MMPLAYER_CREATE_ELEMENT(audiobin, MMPLAYER_A_SINK, player->ini.name_of_audiosink, "audiosink", link_audio_sink_now);
		...
		{
			debug_log("provide clock for video = %s", (player->ini.provide_clock_for_video)?"audio clock":"system clock");
			g_object_set (G_OBJECT (audiobin[MMPLAYER_A_SINK].gst), "provide-clock", player->ini.provide_clock_for_video,  NULL);
		}
		else
		{
			debug_log("provide clock for music = %s", (player->ini.provide_clock_for_music)?"audio clock":"system clock");
...
/* for audio tunning */
			...
			debug_log("audiosink property status...volume type:%d, route:%d, priority=%d, user-route=%d, latency=%d\n",
				volume_type, audio_route, sound_priority, route_path, latency_mode);
		}
...
__mmplayer_gst_create_video_filters(mm_player_t* player, GList** bucket, gboolean use_video_stream)
{
	...
	if (use_video_stream)
	{
		...
		MMPLAYER_CREATE_ELEMENT(player->pipeline->videobin, MMPLAYER_V_CONV, video_csc, "video converter", TRUE);
		...
		if ( !player->set_mode.video_zc)
		{
			...
			MMPLAYER_CREATE_ELEMENT(player->pipeline->videobin, MMPLAYER_V_FLIP, "videoflip", "video rotator", TRUE);
			MMPLAYER_CREATE_ELEMENT(player->pipeline->videobin, MMPLAYER_V_SCALE, "videoscale", "video scaler", TRUE);
			MMPLAYER_CREATE_ELEMENT(player->pipeline->videobin, MMPLAYER_V_CAPS, "capsfilter", "videocapsfilter", TRUE);
	...
	else
	{
		...
		/* videoscaler */
		#if !defined(__arm__)
		MMPLAYER_CREATE_ELEMENT(player->pipeline->videobin, MMPLAYER_V_SCALE, "videoscale", "videoscaler", TRUE);
		#endif
	}
...
static int __mmplayer_gst_create_text_pipeline(mm_player_t* player)
{
	...
	else
	{
		...
		GST_OBJECT_FLAG_UNSET (textbin[MMPLAYER_T_SINK].gst, GST_ELEMENT_IS_SINK);
...
__mmplayer_gst_create_subtitle_src(mm_player_t* player)
{
	...
	GstPad *pad = NULL;
	pad = gst_element_get_static_pad (player->pipeline->textbin[MMPLAYER_T_SINK].gst, "sink");
...
__mmplayer_bus_sync_callback (GstBus * bus, GstMessage * message, gpointer data)
{
        ...
        switch (GST_MESSAGE_TYPE (message))
        {
                ...
                case GST_MESSAGE_DURATION:
                        ...
                        break;
                case GST_MESSAGE_ASYNC_DONE:
                        if( message->src != (GstObject *)mainbin[MMPLAYER_M_PIPE].gst )
                        {
                                gst_message_unref (message);
                                return GST_BUS_DROP;
                        }

                        debug_log ("received asyncdone message");
                        if( player->ignore_asyncdone == TRUE)
                        {
                                debug_log ("DROP async done message");
                                gst_message_unref (message);
                                return GST_BUS_DROP;
                        }
                default:
	...
	switch ( player->profile.uri_type )
	{
		...
		case MM_PLAYER_URI_TYPE_URL_HTTP:
		{
			...
			if ( ! MMPLAYER_IS_HTTP_PD(player) )
			{
				...
				secure_debug_log("user_agent :  %s\n",  user_agent);
				debug_log("timeout : %d\n",  player->ini.http_timeout);

				/* setting property to streaming source */
				g_object_set(G_OBJECT(element), "location", player->profile.uri, NULL);
				g_object_set(G_OBJECT(element), "timeout", player->ini.http_timeout, NULL);
		...
		case MM_PLAYER_URI_TYPE_SS:
		{
			...
			if ( !element )
			{
				debug_critical("failed to create http streaming source element[%s]", player->ini.name_of_httpsrc);
				break;
			}
...
#ifdef _LITEW_OPT_
			newplugin = gst_plugin_load_file(_GST_APPSRC_PATH, NULL);
			if (newplugin) {
				gst_default_registry_add_plugin(newplugin);
				gst_object_unref (newplugin);
			}
			else {
				debug_error("failed to load %s", _GST_APPSRC_PATH);
			}
#endif
...
static int __gst_start(mm_player_t* player) // @
{
	...
	/* early wake up for avsysaudiosink */
	if (player->pipeline->audiobin && player->pipeline->audiobin[MMPLAYER_A_SINK].gst)
	{
		g_object_set (G_OBJECT (player->pipeline->audiobin[MMPLAYER_A_SINK].gst), "open-audio", TRUE, NULL);
	}
...
static int 	__gst_get_buffer_position(mm_player_t* player, int format, unsigned long* start_pos, unsigned long* stop_pos)
{
	...
	if ( MMPLAYER_IS_HTTP_STREAMING ( player ))
	{
		...
		element = GST_ELEMENT ( player->pipeline->mainbin[MMPLAYER_M_MUXED_S_BUFFER].gst );
	}
...
__mmplayer_asm_callback(int handle, ASM_event_sources_t event_src, ASM_sound_commands_t command, unsigned int sound_status, void* cb_data)
{
	...
	{
		debug_warning("player must be destroyed or corrupted\n");
		return cb_res;
	}

	if (player->cmd == MMPLAYER_COMMAND_UNREALIZE || player->cmd == MMPLAYER_COMMAND_DESTROY)
	{
		debug_warning("player has being unrealized or destroyed\n");
		return cb_res;
	}

	MMPLAYER_ASM_CONTEXT_LOCK(player);

	if ( player->state < MM_PLAYER_STATE_NULL || player->state > MM_PLAYER_STATE_NUM )
	{
		debug_warning("player must be destroyed or corrupted. player->state = %d \n", player->state);
		MMPLAYER_ASM_CONTEXT_UNLOCK(player);
		return cb_res;
	}
...
leave:
	MMPLAYER_FLEAVE();
	debug_warning("asm cb dispatched");
...
#endif
	player->ignore_asyncdone = FALSE;
	player->is_audio_excluded = FALSE;
...
__mmplayer_create_decodebin (mm_player_t* player)
{
	...
	MMPLAYER_SIGNAL_CONNECT( player, G_OBJECT(decodebin), "pad-added",
						G_CALLBACK(__mmplayer_gst_decode_pad_added), player);

	/* no-more-pad pad handling signal */
	MMPLAYER_SIGNAL_CONNECT( player, G_OBJECT(decodebin), "no-more-pads",
						G_CALLBACK(__mmplayer_gst_decode_no_more_pads), player);

	MMPLAYER_SIGNAL_CONNECT( player, G_OBJECT(decodebin), "pad-removed",
						G_CALLBACK(__mmplayer_gst_decode_pad_removed), player);

	/* This signal is emitted when a pad for which there is no further possible
	   decoding is added to the decodebin.*/
	MMPLAYER_SIGNAL_CONNECT( player, G_OBJECT(decodebin), "unknown-type",
	...
	MMPLAYER_SIGNAL_CONNECT( player, G_OBJECT(decodebin), "autoplug-continue",
						G_CALLBACK(__mmplayer_gst_decode_autoplug_continue), player);

	/* This signal is emitted whenever decodebin2 finds a new stream. It is emitted
	   before looking for any elements that can handle that stream.*/
	MMPLAYER_SIGNAL_CONNECT( player, G_OBJECT(decodebin), "autoplug-select",
	...
	/* Once decodebin2 has found the possible GstElementFactory objects to try for
	   ...
	   array.*/
	MMPLAYER_SIGNAL_CONNECT( player, G_OBJECT(decodebin), "autoplug-sort",
	...
	MMPLAYER_SIGNAL_CONNECT( player, G_OBJECT(decodebin), "drained",
						G_CALLBACK(__mmplayer_gst_decode_drained), player);

	/* This signal is emitted when a element is added to the bin.*/
	MMPLAYER_SIGNAL_CONNECT( player, G_OBJECT(decodebin), "element-added",
...
__mmplayer_try_to_plug_decodebin(mm_player_t* player, GstPad *srcpad, const GstCaps *caps)
{
		...
		(MMPLAYER_IS_HTTP_STREAMING(player)))
	{
		...
		else
	    {
			...
			// if ( !MMPLAYER_IS_HTTP_LIVE_STREAMING(player))
			{
				...
				if ((dur_bytes>0) && (!__mmplayer_is_only_mp3_type(player->type)))
				{
	...
	* the state change */
	...
	mainbin[MMPLAYER_M_AUTOPLUG].id = MMPLAYER_M_AUTOPLUG;
	mainbin[MMPLAYER_M_AUTOPLUG].gst = decodebin2;	// over write
...
GstCaps * caps,  gpointer data)
{
    ...
    if ( player->is_audio_excluded && g_str_has_prefix(mime, "audio"))
	{
        debug_log("exclude audio...");
        ret = FALSE;
    }
    else {
//        debug_log("found new stream with caps [%s]", caps_str);
...
//	debug_log("found new element [%s] to link for caps [%s]", factory_name, caps_str);
	...
	else if ((g_strrstr(klass, "Codec/Decoder/Video")))
	{
		...
		//debug_log ("Keep Demux Video Pad Caps information = %s\n", caps_str);
...
__mmplayer_gst_decode_drained(GstElement *bin, gpointer data) // @
{
	...
	MMPLAYER_FENTER();
	debug_log("decodebin has decoded all data");
	MMPLAYER_FLEAVE();
}
...
__mmplayer_gst_element_added (GstElement *bin, GstElement *element, gpointer data)
{
	...
	else if (g_strrstr(factory_name, "omx_"))
	{
		if (g_strrstr(klass, "Codec/Decoder/Video"))
		{
			...
			{
				ret = gst_pad_set_caps(gst_element_get_static_pad(element, "sink"), player->v_stream_caps);
...
#ifdef _LITEW_OPT_
	mm_attrs_get_int_by_name(player->attrs, "profile_play_count", &count);

	if (-1 == count)
	{
		debug_log("posting EOS right away");
...
__mmplayer_do_change_videosink(mm_player_t* player, const int dec_index, const char *videosink_element, MMDisplaySurfaceType surface_type, void *display_overlay)
{
	...
	MMTA_ACUM_ITEM_BEGIN("__mmplayer_do_change_videosink", FALSE);
	...
	MMTA_ACUM_ITEM_END("__mmplayer_do_change_videosink", FALSE);
...
__is_streaming ( mm_player_t* player )
{
  ...
  return ( __is_rtsp_streaming ( player ) || __is_wfd_streaming ( player ) || __is_http_streaming ( player ) || __is_http_live_streaming ( player )) ? TRUE : FALSE;
}
