...
_mmplayer_audio_effect_preset_apply(mm_player_t *player, MMAudioEffectPresetType effect_type)
{
	...
	else
	{
		...
		else if (device_out == MM_SOUND_DEVICE_OUT_HDMI || device_out == MM_SOUND_DEVICE_OUT_MIRRORING)
		{
		...
		debug_log("filter-output-mode = %d (0:spk,1:ear,2:others)", output_type);
...
mm_player_audio_effect_preset_apply(MMHandleType hplayer, MMAudioEffectPresetType type)
{
	...
	if (!player->ini.use_audio_effect_preset)
	{
	...
	if ( !_mmplayer_is_supported_effect_type( player, MM_AUDIO_EFFECT_TYPE_PRESET, type ) )
	{
		MMPLAYER_FLEAVE();
