...
int mm_player_create(MMHandleType *player)
{
	...
	/* create asm context lock */
	new_player->sm.asm_context_lock = g_mutex_new();

	if ( ! new_player->sm.asm_context_lock )
	{
		debug_error("failed to create asm_context_lock lock\n");
		goto ERROR;
	}
...
int  mm_player_destroy(MMHandleType player)
{
	...
	if (((mm_player_t*)player)->sm.asm_context_lock)
	{
		g_mutex_free(((mm_player_t*)player)->sm.asm_context_lock);
		((mm_player_t*)player)->sm.asm_context_lock = NULL;
	}
...
int mm_player_set_external_subtitle_path(MMHandleType player, const char* path)
{
	...
	return_val_if_fail(player, MM_ERROR_PLAYER_NOT_INITIALIZED);
	return_val_if_fail(path, MM_ERROR_COMMON_INVALID_ARGUMENT);
